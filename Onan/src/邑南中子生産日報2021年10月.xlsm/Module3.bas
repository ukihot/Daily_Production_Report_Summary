...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   ...
   Loop
    ...
    'ショット数集計ファイルを読み出し
    Set wb = Workbooks.Open(Filename:=ThisWorkbook.Path & "\..\ショット管理表\【邑南】ショット数集計.xls ")
    ...
    End If
    'ショット数を入力していく列を検索
    Set temp = wb.Worksheets("邑南中子工場").Range("J3")
    Do While temp.Value <> ""
    ...
    ThisWorkbook.Worksheets("品名別集計").Range("B7:B99").Interior.ColorIndex = 2
    
    Set 元品番 = ThisWorkbook.Worksheets("品名別集計").Range("B7")
    Do While 元品番.Value <> ""
        '先品番の初期化
        Set 先品番 = wb.Worksheets("邑南中子工場").Range("B6")
        '先品番の検索
        ...
        Loop
        '値の書き換え
        With wb.Worksheets("邑南中子工場")
            .Cells(先品番.Row, ActiveCell.Column).Value = ThisWorkbook.Worksheets("品名別集計").Cells(元品番.Row, 3).Value
            .Cells(先品番.Row, ActiveCell.Column).Font.ColorIndex = 1
        End With
        'チェック用
        元品番.Interior.ColorIndex = 3
...
rt1:
    ...
    '平均ショット数（過去6ヶ月）更新  '20121001追加
    Set 先品番 = wb.Worksheets("邑南中子工場").Range("B6")
    Do Until 先品番 = ""
        If 先品番 <> 先品番.Offset(-1, 0) Then
            With wb.Worksheets("邑南中子工場")
                '平均ショット数（過去6ヶ月）
                .Cells(先品番.Row, 6).FormulaR1C1 = "=sum(RC[" & temp.Column - 6 - 5 & "]:RC[" & temp.Column - 6 - 0 & "])/6"
            End With
        End If
        Set 先品番 = 先品番.Offset(1, 0)
    Loop
    ...
    For s = 1 To 6  '6ヶ月（半年）分処理を繰り返す
        ...
        If fm > 12 Then
            fm = fm Mod 12
            'If t = 0 Then  '20121203修正　年度が正しく表示されず、何のためのif文か分からない
                fy = fy + 1
                t = t + 1
            'End If
        End If
    ...
    '修正チェック
    Application.Run "【邑南】ショット数集計.xls!修正check"
